...
type UsersTableNavigationProp = NativeStackNavigationProp<
  RootStackParamList,
  'Edit Users'
>;
...
const EditUsersPage = ({route}: Props) => {
  ...
  const filteredUsers = users
    ...
    .sort((a: {firstName: string}, b: {firstName: string}) => {
      if (a.firstName.toLowerCase().startsWith(searchQuery.toLowerCase()))
        return -1;
      if (b.firstName.toLowerCase().startsWith(searchQuery.toLowerCase()))
        return 1;
      return 0;
    });
  ...
  const onDeletePressed = async (email: string) => {
    try {
      ...
      console.error(`${error} EditUsersPage ${URL}/user/getUser`);
